"""Test the hello function and invert function."""
...
def test_invert() -> None:
    ...
    inverted = nextcv.image.invert(data)
...
def test_invert_edge_cases() -> None:
    """Test invert function with edge cases like empty arrays."""
    # Test with empty array
    empty = np.array([], dtype=np.uint8)
    empty_inverted = nextcv.image.invert(empty)
    assert len(empty_inverted) == 0

    # Test with all zeros
    zeros = np.zeros(5, dtype=np.uint8)
    zeros_inverted = nextcv.image.invert(zeros)
    assert np.all(zeros_inverted == np.iinfo(np.uint8).max)

    # Test with all 255s
    max_vals = np.full(5, 255, dtype=np.uint8)
    max_inverted = nextcv.image.invert(max_vals)
    assert np.all(max_inverted == np.iinfo(np.uint8).min)


def test_invert_2d() -> None:
    ...
    inverted_2d = nextcv.image.invert(data_2d)
...
def test_invert_3d() -> None:
    """Test invert function with 3D arrays (e.g., RGB images)."""
    # Test with a simple 3D array (2x2x3 RGB image)
    data_3d = np.array(
        [[[0, 128, 255], [64, 192, 32]], [[255, 0, 128], [96, 160, 224]]],
        dtype=np.uint8,
    )
    inverted_3d = nextcv.image.invert(data_3d)
    ...
    expected_3d = np.array(
        [[[255, 127, 0], [191, 63, 223]], [[0, 255, 127], [159, 95, 31]]],
        dtype=np.uint8,
    )
    assert np.array_equal(inverted_3d, expected_3d)


def test_invert_4d() -> None:
    ...
    data_4d = rng.integers(0, 256, size=(2, 2, 2, 2), dtype=np.uint8)
    inverted_4d = nextcv.image.invert(data_4d)
