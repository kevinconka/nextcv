...
py::array_t<std::uint8_t> invert_numpy(const py::array_t<std::uint8_t>& input) {
...
PYBIND11_MODULE(nextcv_py, m) {
    ...
    m.def("invert_numpy", &invert_numpy, "Invert 8-bit pixels provided as numpy array, returns numpy array");
}
